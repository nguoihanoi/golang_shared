...
type User struct {
	...
	ResetTokenExp    time.Time `bson:"reset_token_exp,omitempty" json:"reset_token_exp"`
...
type UserMeta struct {
	ID              string    `bson:"_id,omitempty" json:"_id,omitempty"`
	TwoFactorSecret string    `bson:"two_factor_secret,omitempty" json:"-"` // Secret key for 2FA (TOTP)
	BackupCodes     []string  `bson:"backup_codes,omitempty" json:"-"`      // Backup codes for 2FA recovery
	CreatedAt       time.Time `bson:"created_at" json:"created_at"`
	UpdatedAt       time.Time `bson:"updated_at" json:"updated_at"`
}
